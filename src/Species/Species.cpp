...
Species::Species( Params &params, Patch *patch ) :
    c_part_max_( 1 ),
    ionization_rate_( Py_None ), // all these member variables (ionization_rate_, pusher_name_, etc.) 
    pusher_name_( "boris" ), // are initialized before the body of the C-TOR is executed, 
    radiation_model_( "none" ), // i.e. are initialized using a member-initializing list.
    time_frozen_( 0 ), // It is faster than assignment inside the body of the C-TOR
    radiating_( false ), // because here we only call the C-TOR once.
    relativistic_field_initialization_( false ), // and the object will be created and constructed in 1 operation
    time_relativistic_initialization_( 0 ), 
    multiphoton_Breit_Wheeler_( 2, "" ), // When using asignement inside the body of C-TOR:
    ionization_model( "none" ), // the fields will be first initialized with default constructors
    density_profile_type_( "none" ), // and then reassigned (via assignment operator) with actual values.
    charge_profile_( NULL ), // Thus 2 calls of C-TORs! Thus a significant overhead for user-defined Classes.
