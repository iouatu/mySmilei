...
public:
    static Species *create( Params &params, int ispec, Patch *patch )
    {
        ...
        Species *this_species = NULL; // THIS WILL BE RETURNED AT END
        ...
        if( this_species->mass_ > 0 ) { // if this species is not composed of photons.
            ...
            std::string model;
            PyTools::extract( "ionization_model", model, "Species", ispec ); // PyTools::extract() function implementation to extract a string.
            
            if(model != "none") { // else: nothing happens, this species is not ionizable?
                ...
                if(model == "tunnel" || model == "tunnel_BSI"){
                    if (params.Laser_Envelope_model) ERROR("An envelope is present, so tunnel_envelope or tunnel_envelope_averaged ionization model should be selected for species "<<species_name);
                ...
                PyTools::extract( "ionization_electrons", this_species->ionization_electrons, "Species", ispec ); // the species where the ionized electrons are going to after being ionized from the atom/ion.

            } // END if( model != "none" ) scope

        } // END "Manage the ionization parameters" section of this code-file.
